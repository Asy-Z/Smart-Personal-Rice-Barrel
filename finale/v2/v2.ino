...
float weight;
float calibration_factor = 291843; // Adjust this value based on your calibration

// Variable to store the system activation state
bool systemActive = false;

BLYNK_WRITE(V2) {
  // Read the value from the Blynk app toggle switch
...
void measureWeight() {
  // Only proceed with weight measurement if the system is active
  if (systemActive) {
    ...
    // System is not active, perform any necessary actions when the system is off
    // For example, you might want to turn off devices or sensors
